...
import { CSHC, DEEMING, DSP, PENSION_ASSETS_TEST } from "@/lib/constants/centrelink-means-test";
...
{/* ── Calculators (added 2026-08-28: spokes for the payment types this hub was catching) ── */}
          <section>
            ...
            <ul className="list-disc pl-6 space-y-2 text-warmgray">
              ...
              <li><Link href="/age-pension-assets-test-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Age Pension assets test calculator</Link> &mdash; full pension up to {formatAUD(PENSION_ASSETS_TEST.fullPensionLimit.single.homeowner)} (single homeowner), $3 less per $1,000 above it, and which test sets your pension.</li>
              <li><Link href="/deeming-rates/" className="text-eucalyptus-dark hover:underline font-medium">Deeming rates and calculator</Link> &mdash; {(DEEMING.lowerRate * 100).toFixed(2)}% and {(DEEMING.upperRate * 100).toFixed(2)}% from {DEEMING.ratesFrom}, and what your savings add to the income test.</li>
              <li><Link href="/disability-support-pension-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Disability Support Pension calculator</Link> &mdash; the pension income test on your wages and the {DSP.maxWorkHoursPerWeek}-hours-a-week work rule.</li>
              <li><Link href="/child-care-subsidy-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Child Care Subsidy calculator</Link> &mdash; {CCS.standard.maxPercent}% up to {formatAUD(CCS.standard.lowerThreshold)} family income, 1% less per {formatAUD(CCS.standard.step)} above, and your gap fee.</li>
              <li><Link href="/commonwealth-seniors-health-card/" className="text-eucalyptus-dark hover:underline font-medium">Commonwealth Seniors Health Card</Link> &mdash; income under {formatAUD(CSHC.incomeLimit.single)} single or {formatAUD(CSHC.incomeLimit.couple)} couple, no assets test.</li>
              {/* end H3 */}
              {/* end W3 */}
            </ul>
...
{/* ── H2 3: Income Test Thresholds Table — every figure from lib/constants/centrelink-income-test.ts ── */}
          ...
          <section>
            ...
            <div className="not-prose my-6"><div className="overflow-hidden rounded-xl border border-sandstone-dark/20 shadow-sm"><table className="w-full text-sm text-left text-warmgray"><thead className="bg-sandstone font-semibold text-navy"><tr><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Homeowner &mdash; Full Pension Limit</th><th className="px-5 py-3 text-right">Non-Homeowner &mdash; Full Pension Limit</th></tr></thead><tbody className="divide-y divide-sandstone-dark/20 bg-white">
              <tr><td className="px-5 py-3">Single</td><td className="px-5 py-3 text-right">{formatAUD(PENSION_ASSETS_TEST.fullPensionLimit.single.homeowner)}</td><td className="px-5 py-3 text-right">{formatAUD(PENSION_ASSETS_TEST.fullPensionLimit.single.nonHomeowner)}</td></tr>
              <tr><td className="px-5 py-3">Couple (combined)</td><td className="px-5 py-3 text-right">{formatAUD(PENSION_ASSETS_TEST.fullPensionLimit.couple.homeowner)}</td><td className="px-5 py-3 text-right">{formatAUD(PENSION_ASSETS_TEST.fullPensionLimit.couple.nonHomeowner)}</td></tr>
            </tbody></table></div></div>
            <p>Assets above the full-pension limit reduce the Age Pension by <strong>$3.00 per fortnight</strong> for every <strong>$1,000</strong> of assets above the threshold (single rate) or <strong>$1.50 each</strong> for couples. The assets test does not count your principal home, but it does count superannuation balances if you have reached Age Pension age. From {PENSION_ASSETS_TEST.ratesFrom} a part pension stops above {formatAUD(PENSION_ASSETS_TEST.partPensionCutOff.single.homeowner)} (single homeowner) or {formatAUD(PENSION_ASSETS_TEST.partPensionCutOff.couple.homeowner)} (couple homeowner). Run both tests on your own figures with the <Link href="/age-pension-assets-test-calculator/">Age Pension assets test calculator</Link>.</p>
...
{/* ── H2: Working Credit — kept to what Services Australia states ── */}
          ...
          <section>
            ...
            <ul>
              ...
              {/* end W3 */}
              {/* H3 */}
              <li><Link href="/age-pension-assets-test-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Age Pension Assets Test Calculator</Link> &mdash; both pension tests with your wages and savings</li>
              <li><Link href="/deeming-rates/" className="text-eucalyptus-dark hover:underline font-medium">Deeming Rates</Link> &mdash; current rates, thresholds and a deeming calculator</li>
              <li><Link href="/disability-support-pension-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Disability Support Pension Calculator</Link> &mdash; DSP rates, income test and work hours</li>
              <li><Link href="/child-care-subsidy-calculator/" className="text-eucalyptus-dark hover:underline font-medium">Child Care Subsidy Calculator</Link> &mdash; CCS on your family income and your gap fee</li>
              <li><Link href="/commonwealth-seniors-health-card/" className="text-eucalyptus-dark hover:underline font-medium">Commonwealth Seniors Health Card</Link> &mdash; the income limit and an eligibility check</li>
              {/* end H3 */}
          ...
          <section>
            ...
            <Accordion type="multiple" className="not-prose mt-6 space-y-3">
              ...
              <AccordionItem value="deeming" className="border rounded-lg px-4 bg-white">
                ...
                <AccordionContent className="text-warmgray">Deeming is a method Centrelink uses to assess income from financial assets (bank accounts, shares, managed funds, superannuation in pension phase). Rather than counting actual returns, Centrelink applies a fixed &ldquo;deemed&rdquo; rate. From {DEEMING.ratesFrom}, for singles, the first <strong>{formatAUD(DEEMING.thresholds.single)}</strong> is deemed at <strong>{(DEEMING.lowerRate * 100).toFixed(2)}%</strong> and any balance above that at <strong>{(DEEMING.upperRate * 100).toFixed(2)}%</strong>. For a couple where at least one gets a pension, the lower rate applies on the first <strong>{formatAUD(DEEMING.thresholds.pensionerCouple)}</strong> combined. The rates were 1.25% and 3.25% from 20 March to 19 September 2026. The deemed income is added to your other assessable income for the income test. See <Link href="/deeming-rates/">deeming rates and calculator</Link> for the full history and your own figures.</AccordionContent>
              </AccordionItem>
        ...
        <aside className="lg:w-1/3"><div className="sticky top-8 space-y-6"><Card className="bg-sandstone border-sandstone-dark/20"><CardContent className="p-6"><h3 className="font-bold text-navy mb-3">Related</h3><div className="space-y-3"><SidebarLink href="/jobseeker-payment-calculator/" label="JobSeeker Payment Calculator" /><SidebarLink href="/austudy-youth-allowance-calculator/" label="Austudy &amp; Youth Allowance Calculator" /><SidebarLink href="/age-pension-income-test-calculator/" label="Age Pension Income Test Calculator" /><SidebarLink href="/parenting-payment-calculator/" label="Parenting Payment Calculator" /><SidebarLink href="/family-tax-benefit-calculator/" label="Family Tax Benefit Calculator" /><SidebarLink href="/rent-assistance-calculator/" label="Rent Assistance Calculator" /><SidebarLink href="/carer-payment-calculator/" label="Carer Payment Calculator" /><SidebarLink href="/carer-allowance/" label="Carer Allowance" /><SidebarLink href="/centrelink-advance-payment/" label="Centrelink Advance Payment" /><SidebarLink href="/centrelink-payment-dates/" label="Centrelink Payment Dates" /><SidebarLink href="/centrelink-crisis-payment/" label="Crisis Payment" /><SidebarLink href="/centrelink-debt/" label="Centrelink Debt" /><SidebarLink href="/age-pension-assets-test-calculator/" label="Age Pension Assets Test" /><SidebarLink href="/deeming-rates/" label="Deeming Rates" /><SidebarLink href="/disability-support-pension-calculator/" label="Disability Support Pension" /><SidebarLink href="/child-care-subsidy-calculator/" label="Child Care Subsidy Calculator" /><SidebarLink href="/commonwealth-seniors-health-card/" label="Commonwealth Seniors Health Card" /><SidebarLink href="/take-home-pay-calculator/" label="Take-Home Pay Calculator" /><SidebarLink href="/tax-brackets/" label="Tax Brackets Guide" /><SidebarLink href="/parental-leave-pay/" label="Paid Parental Leave Calculator" /><SidebarLink href="/income-tax-calculator/" label="Income Tax Calculator" /><SidebarLink href="/low-income-tax-offset/" label="Low Income Tax Offset Guide" /><SidebarLink href="/superannuation-calculator/" label="Superannuation Calculator" /></div></CardContent></Card></div></aside>
      </div>
    </div></div>
  );
}
