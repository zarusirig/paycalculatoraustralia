...
export default function SaptoPage() {
  return (
    <div className="min-h-screen flex-grow bg-white"><div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <header className="mb-10 max-w-4xl"><h1 className="text-4xl md:text-5xl font-extrabold text-navy leading-tight mb-6" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>SAPTO Calculator &amp; Eligibility Checker <span className="block text-2xl md:text-3xl font-bold text-warmgray mt-2">Seniors and Pensioners Tax Offset</span></h1><p className="text-xl text-warmgray leading-relaxed mb-6">SAPTO, the seniors and pensioners tax offset, is a non-refundable offset for people who receive an Australian Government pension or allowance or who qualify for the Age Pension without claiming it. For {SAPTO_INCOME_YEAR} the maximum is <strong>{formatAUD(S.maxOffset)}</strong> single and <strong>{formatAUD(C.maxOffset)}</strong> for each partner of a couple, reducing by {SAPTO_REDUCTION_RATE * 100} cents per dollar of rebate income above {formatAUD(S.shadingOutThreshold)} or {formatAUD(C.shadingOutThreshold)}. Work out your exact entitlement below, or check whether you qualify with the <a href="#sapto-eligibility" className="text-eucalyptus-dark underline">eligibility checker</a>.</p><TrustBar className="!max-w-none" /></header>
