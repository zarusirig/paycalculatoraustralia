...
export async function generateStaticParams() {
  return serviceStateParams("firefighter");
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { state } = await params;
  return serviceStateMetadata("firefighter", state);
}

async function Page({ params }: PageProps) {
  const { state } = await params;
  return <ServiceStateRoute occupation="firefighter" slug={state} />;
}

export default withPageEnd(Page, "/firefighter-pay/[state]/");
